...
if (typeof document !== 'undefined' && !document.getElementById('cp-keyframes')) {
  ...
  s.textContent = `
    ...
    @keyframes pulse-ring   { 0%{ box-shadow: 0 0 0 0 rgba(99,102,241,.55); } 100%{ box-shadow: 0 0 0 18px rgba(99,102,241,0); } }
    ...
    .cp-tr:hover td { background: rgba(99,102,241,.05) !important; }
    ...
    .cp-action-btn:hover  { background: rgba(56,189,248,.2) !important; transform: scale(1.04); }
  `;
...
function getInitials(id = '') { return id.replace(/[^a-zA-Z0-9]/g,'').slice(0,2).toUpperCase() || '??'; }

// Satisfaction display: 1-10 as coloured dots
...
// Small stat card in the hero strip
      ...
      {!loading && !customer && !notFound && !error && (
        <div style={S.emptyState}>
          ...
          <p style={{ margin:'0 0 20px', color:'#64748b', maxWidth:'400px', textAlign:'center', lineHeight:1.6 }}>
            Enter a Customer ID above and click <strong style={{ color:'#818cf8' }}>Search Profile</strong> to view detailed
            behavioral audit, model predictions, and explainability factors.
          </p>
          <div style={S.exampleIds}>
            <span style={{ color:'#64748b', fontSize:'.82rem' }}>Try:</span>
            {['C10001','C10239','C10500'].map(id=>(
              <button key={id} onClick={()=>{setSearchId(id); setCustomerId(id); if(setSelectedCustomerId) setSelectedCustomerId(id); else fetchCustomerDetails(id);}} style={S.exampleIdBtn}>{id}</button>
            ))}
          </div>
        </div>
      )}
      ...
      {!loading && customer && (
        <div style={{ animation:'fadeSlideUp .4s ease both' }}>
          ...
          <div className="cp-card" style={S.heroBanner}>
            ...
            {/* Risk Gauge (if prediction exists) */}
            {prediction && (
              <div style={S.heroGauge}>
          ...
          <div style={S.kpiStrip}>
            ...
            <StatPill icon="⭐" label="Satisfaction"  value={`${customer.satisfaction_score}/10`} color={customer.satisfaction_score <= 3 ? '#ef4444' : customer.satisfaction_score <= 6 ? '#f59e0b' : '#10b981'} />
            {prediction && (
          ...
          <div style={S.mainGrid}>
            ...
            <div style={S.leftCol}>
              ...
              <div className="cp-card" style={S.card}>
                <h3 style={S.cardTitle}>
                  <span style={S.cardTitleDot}/>
                  Core Customer Profile
                </h3>

                <div style={S.profileTable}>
                  {[
                    ...
                    { label:'Income Level',   value:customer.income_level, badge: customer.income_level },
              ...
              <div className="cp-card" style={S.card}>
                ...
                <MetricBar label="Monthly Spend"          value={Number(customer.monthly_total_spend).toFixed(0)}      max={250} unit=""    color="#a78bfa" delay={.05} />
                <MetricBar label="App Switch Frequency"   value={customer.app_switch_frequency}     max={50}  unit="/hr" color="#fb923c" delay={.10} />
                <MetricBar label="Support Interactions"   value={customer.customer_support_interactions} max={15} unit=""  color="#f87171" delay={.15} />
                <MetricBar label="Active Subscriptions"   value={customer.number_of_subscriptions}  max={10}  unit=""    color="#34d399" delay={.20} />
                <MetricBar label="Tenure Months"          value={customer.tenure_months}             max={60}  unit="m"   color="#06b6d4" delay={.25} />
              ...
              <div className="cp-card" style={S.card}>
                <h3 style={S.cardTitle}>
                  <span style={{ ...S.cardTitleDot, background:'#f59e0b' }}/>
                  Segment Benchmarks
                </h3>
                <p style={S.helperText}>How this customer compares to the {customer.income_level}-income cohort averages:</p>
                <div style={S.benchmarkTable}>
                  {[
                    { metric:'Satisfaction',  customer:customer.satisfaction_score, avg:5.2, higher_is_better:true, unit:'/10' },
                    { metric:'Weekly Usage',  customer:customer.avg_usage_hours_per_week, avg:14.8, higher_is_better:true, unit:'h' },
                    { metric:'Monthly Spend', customer:Number(customer.monthly_total_spend), avg:72.40, higher_is_better:true, unit:'$', prefix:'$' },
                    { metric:'Support Calls', customer:customer.customer_support_interactions, avg:2.1, higher_is_better:false, unit:'' },
                  ].map(({ metric, customer: cv, avg, higher_is_better, unit, prefix='' })=>{
                    const diff = cv - avg;
                    const better = higher_is_better ? diff >= 0 : diff <= 0;
                    const col = better ? '#34d399' : '#f87171';
                    return (
                      <div key={metric} style={S.benchmarkRow}>
                        <span style={{ fontSize:'.85rem', color:'#94a3b8', width:'110px' }}>{metric}</span>
                        ...
                        <span style={{ fontSize:'.8rem', fontWeight:700, color:col }}>
            ...
            <div style={S.rightCol}>
              ...
              {prediction && (
                <div className="cp-card" style={{ ...S.recommendationCard, marginBottom:'0' }}>
                  <div style={S.recommendationHeader}>
                    <div>
                      <h4 style={S.recommendationTitle}>
                        {prediction.will_cancel === 1 ? '🚨' : '✅'} Retention Recommendation
                      </h4>
                      <p style={S.recommendationSub}>Based on ML model output and behavioral signals</p>
                    </div>
                    <div style={{ ...S.recommendationBadge, background:`${riskColor}22`, color:riskColor, border:`1px solid ${riskColor}44` }}>
                      {prediction.recommendation_type}
                    </div>
                  </div>
                  <p style={S.recommendationDesc}>{prediction.recommendation_desc}</p>
                </div>
              )}
              ...
              {prediction?.explainability && (
                <div className="cp-card" style={{ marginBottom:'0' }}>
                  <h3 style={S.cardTitle}>
                    <span style={{ ...S.cardTitleDot, background:'#a78bfa' }}/>
                    Local SHAP Feature Importance
                  </h3>
                  <p style={S.helperText}>
                    Per-feature SHAP values: <span style={{ color:'#f87171' }}>red = increases churn risk</span>,&nbsp;
                    <span style={{ color:'#34d399' }}>green = reduces churn risk</span>
                  </p>
                        ...
                        return (
                          <div key={key} style={{ display:'grid', gridTemplateColumns:'160px 1fr 54px', gap:'10px', alignItems:'center', fontSize:'.84rem' }}>
                            ...
                            <div style={{ background:'rgba(255,255,255,.06)', height:'8px', borderRadius:'4px', overflow:'hidden' }}>
                              <div className="cp-bar" style={{
                                width:`${widthPct}%`, height:'100%', borderRadius:'4px',
              ...
              <div className="cp-card" style={{ marginBottom:'0' }}>
                <h3 style={S.cardTitle}>
                  <span style={{ ...S.cardTitleDot, background:'#06b6d4' }}/>
                  Model Audit Log
                </h3>
                {history.length > 0 ? (
                  <div style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
                    {history.map((log, idx)=>(
                      <div key={log.history_id} className="cp-tr" style={S.auditRow}>
                        <div style={{ display:'flex', flexDirection:'column', gap:'2px' }}>
                          <span style={{ color:'#94a3b8', fontSize:'.8rem' }}>
                            {new Date(log.evaluated_at).toLocaleString()}
                          </span>
                          {idx === 0 && <span style={S.auditLatest}>Latest Run</span>}
                        </div>
                        <span style={{ ...S.auditBadge, background: log.prediction_result===1 ? 'rgba(239,68,68,.12)' : 'rgba(16,185,129,.12)', color: log.prediction_result===1 ? '#fca5a5' : '#a7f3d0', border:`1px solid ${log.prediction_result===1?'rgba(239,68,68,.25)':'rgba(16,185,129,.25)'}` }}>
                          {log.prediction_result===1?'Churn':'Retain'}
                        </span>
                        <span style={{ fontWeight:700, color:'#e2e8f0', fontSize:'.9rem' }}>
                          {formatPercent(log.risk_score)} Risk
                        </span>
                        <span style={{ ...S.auditRiskBadge(log.risk_category) }}>
                          {log.risk_category}
                        </span>
                      </div>
                    ))}
                  ...
                  <p style={S.placeholder}>No historical prediction runs recorded for this profile.</p>
                )}
              </div>
...
const S = {
  ...
  searchInputWrap: { flex:1, display:'flex', alignItems:'center', background:'rgba(15,23,42,.9)', border:'1px solid rgba(255,255,255,.1)', borderRadius:'12px', padding:'0 14px', gap:'10px', transition:'border-color .2s' },
  searchIcon:      { fontSize:'1rem', lineHeight:1 },
  ...
  notFoundCard:{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 20px', background:'rgba(17,24,39,.7)', border:'1px solid rgba(255,255,255,.08)', borderRadius:'20px' },
  emptyState:  { display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'80px 20px', gap:'4px' },
  exampleIds:  { display:'flex', alignItems:'center', gap:'8px', marginTop:'4px' },
  ...
  heroGauge:  { marginLeft:'auto', flexShrink:0 },
  ...
  cardTitleDot: { width:'8px', height:'8px', borderRadius:'50%', background:'#6366f1', flexShrink:0 },

  // Profile table
  profileTable: { display:'flex', flexDirection:'column', gap:'2px' },
  ...
  // Benchmark
  benchmarkTable: { display:'flex', flexDirection:'column', gap:'8px' },
  benchmarkRow:   { display:'grid', gridTemplateColumns:'110px 1fr 80px 60px', gap:'8px', alignItems:'center', padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,.05)' },

  // Recommendation
  recommendationCard:   { background:'rgba(17,24,39,.85)', border:'1px solid rgba(255,255,255,.09)', borderRadius:'18px', padding:'22px 24px', boxShadow:'0 10px 32px rgba(0,0,0,.22)' },
  recommendationHeader: { display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'16px', marginBottom:'14px', flexWrap:'wrap' },
  recommendationTitle:  { margin:'0 0 4px', fontSize:'1.05rem', fontWeight:700, color:'#e2e8f0' },
  recommendationSub:    { margin:0, fontSize:'.78rem', color:'#64748b' },
  recommendationBadge:  { padding:'6px 14px', borderRadius:'20px', fontWeight:700, fontSize:'.82rem', whiteSpace:'nowrap' },
  recommendationDesc:   { margin:0, color:'#cbd5e1', fontSize:'.9rem', lineHeight:1.65 },

  // Audit Log
  auditRow:    { display:'grid', gridTemplateColumns:'1fr auto auto auto', gap:'12px', alignItems:'center', padding:'10px 8px', borderRadius:'8px', transition:'background .18s', cursor:'default' },
  auditBadge:  { padding:'3px 10px', borderRadius:'6px', fontSize:'.76rem', fontWeight:700 },
  auditLatest: { fontSize:'.7rem', color:'#818cf8', fontWeight:600 },
  auditRiskBadge: (cat) => ({
    padding:'3px 8px', borderRadius:'6px', fontSize:'.72rem', fontWeight:700,
    background: cat==='High'?'rgba(239,68,68,.12)':cat==='Medium'?'rgba(245,158,11,.12)':'rgba(16,185,129,.12)',
    color:       cat==='High'?'#fca5a5'           :cat==='Medium'?'#fde047'               :'#a7f3d0',
  }),

  placeholder: { color:'#64748b', fontStyle:'italic', textAlign:'center', padding:'20px 0', margin:0 },
};
